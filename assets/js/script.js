...
function fetchForecast(lat, lon) {
  fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=imperial&appid=fd9ef7dbe0229fd31347b03516a3d415`
  )
...
function createForecastCards(data) {
  ...
  if (searchInputValue) {
    ...
    forecastHeader.textContent = `5-Day Forecast in ${searchInputValue}`;
  ...
  for (let i = 5; i < data.list.length; i += 8) {
    ...
    forecastTemp.textContent = `Temp: ${Math.floor(
      data.list[0].main.temp_max
    )}°F`;

    const forecastWind = document.createElement("p");
    forecastWind.textContent = `Wind: ${Math.floor(
